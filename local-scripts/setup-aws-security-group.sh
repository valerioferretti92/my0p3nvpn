...
then
    aws ec2 create-security-group --group-name $SECURITY_GROUP_NAME --description "SSH and UDP on port 1194 from anywhere"
    aws ec2 authorize-security-group-ingress --group-name $SECURITY_GROUP_NAME --protocol tcp --port 22 --cidr 0.0.0.0/0
    aws ec2 authorize-security-group-ingress --group-name $SECURITY_GROUP_NAME --protocol udp --port 1194 --cidr 0.0.0.0/0
else
